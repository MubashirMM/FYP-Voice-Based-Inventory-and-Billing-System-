...
from myapp.crud.udhaar_item import (
    ...
    delete_udharitem,
    list_udharitems,
    get_udharitems_by_customer_optimized,
    get_udharitem_by_id_optimized,
    search_udharitems_optimized
)
...
@router.get("/", response_model=List[UdharRead])
async def get_all_udharitems(
    limit: int = Query(100, ge=1, le=500, description="نتائج کی تعداد"),
    offset: int = Query(0, ge=0, description="شروع سے چھوڑیں"),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """Optimized with pagination"""
    return await list_udharitems(db, current_user, limit=limit, offset=offset)
...
async def get_udharitems_by_customer(
    ...
    """OPTIMIZED - Direct database query instead of Python filtering"""
    return await get_udharitems_by_customer_optimized(db, customer_name, current_user)
...
async def get_udharitem(
    ...
    """OPTIMIZED - Direct single query"""
    return await get_udharitem_by_id_optimized(db, item_id, current_user)
...
@router.get("/search/", response_model=List[UdharRead])
async def search_items(
    keyword: str = Query(..., min_length=1, description="تلاش کا لفظ"),
    limit: int = Query(50, ge=1, le=100, description="نتائج کی حد"),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """OPTIMIZED - Direct database search instead of Python filtering"""
    return await search_udharitems_optimized(db, keyword, current_user, limit)