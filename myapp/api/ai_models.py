...
from myapp.crud.ai_models.ai_models_reports import process_voice_reports
...
@router.post("/voice-process-reports")
async def voice_process_endpoint(
    payload: VoiceItemsRequest,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user)
    ...
    """
    try:
        result = await process_voice_reports(payload.audio_base64, current_user, db)
