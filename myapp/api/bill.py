...
async def bill_history(customer_id: int, db: AsyncSession = Depends(get_db)):
    res=await get_bills_by_customer(db, customer_id)
    if not res:
...
async def get_bills(db: AsyncSession = Depends(get_db)):
    res= await get_all_bills(db)
    if not res:
        raise HTTPException(status_code=404,detail="اس گاہک کا کوئی غیر ادا شدہ بل موجود نہیں")
    return res

@router.put("/customer/{customer_id}/pay", response_model=BillRead)
async def pay_customer_bill(customer_id: int, db: AsyncSession = Depends(get_db)):
    bill = await pay_bill(db, customer_id)
    if not bill:
        raise HTTPException(status_code=404, detail="اس گاہک کا کوئی غیر ادا شدہ بل موجود نہیں")
    return bill
...
async def delete_bill_endpoint(bill_id: int, db: AsyncSession = Depends(get_db)):
    success = await delete_bill(db, bill_id)
    if not success:
