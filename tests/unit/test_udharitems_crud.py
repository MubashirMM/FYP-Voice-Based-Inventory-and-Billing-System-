# # C:\FYP\Backend\fast-api\tests\unit\test_udhar_crud.py
# import sys
...
#     assert result["requested_unit"] == "KG"
#     assert result["total_amount"] == 500  # 5 * 100
    
#     # Verify stock was reduced
#     from myapp.crud.item import read_item
...
#     from myapp.crud.udhar_item import get_or_create_customer
...
#     """Test creating udhar with different unit (e.g., grams instead of KG)"""
#     user, item = await create_test_user_and_item(db_session)
#     background_tasks = BackgroundTasks()
    
#     # Create item in KG, but request in GRAM
#     result = await create_udhar(
#         db=db_session,
#         customer_name="Test Customer",
#         item_name="Test Product",
...
#             item_name="Test Product",
#             quantity=2,
#             unit="DOZEN",  # Incompatible with KG
#             current_user=user,
#             background_tasks=background_tasks
#         )
...
#     from myapp.crud.item import update_items
#     from myapp.schemas.items import ItemUpdate
...
#     background_tasks = BackgroundTasks()
#     background_tasks.add_task = MagicMock()  # Mock the add_task method
    
#     with patch('myapp.crud.udhar_item.send_email') as mock_send_email:
#         result = await create_udhar(
#             db=db_session,
#             customer_name="Test Customer",
#             item_name="Test Product",
...
#         # Verify email was queued (check if add_task was called)
#         # Note: This might need adjustment based on your implementation
#         assert result is not None
...
#     # Verify stock was adjusted (5 returned, 3 taken = net -2 from original)
#     from myapp.crud.item import read_item
#     updated_item = await read_item(db_session, item.item_id, user)
...
#     # Original item: 5 returned, so stock should be 50 + 5 = 55? Wait, check logic
#     # Actually: Original took 5, now returning 5 and taking 2 from new item
#     assert item1_updated.stock_quantity == 55  # 50 + 5 returned
...
#     update_data = MagicMock()
#     update_data.item_name = "Test Product"
#     update_data.quantity = 60  # More than available (50 - 2 = 48 available)
#     update_data.unit = "KG"
...
#     from myapp.crud.item import read_item
#     updated_item = await read_item(db_session, item.item_id, user)
#     assert updated_item.stock_quantity == 50  # Back to original
...
#     from myapp.crud.item import read_item
#     updated_item = await read_item(db_session, item.item_id, user)
#     assert updated_item.stock_quantity == initial_stock
...
#     async def create_udhar_task():
#         try:
#             return await create_udhar(
#                 db=db_session,
#                 customer_name=f"Customer_{id(asyncio.current_task())}",
#                 item_name="Test Product",
#                 quantity=10,
#                 unit="KG",
#                 current_user=user,
...
#     # Try to create 3 udhars concurrently (total 30 items, stock is 50)
#     tasks = [create_udhar_task() for _ in range(3)]
...
from myapp.crud.user import register_user
from myapp.crud.items import create_items, read_item, update_items
...
async def create_test_user_and_item(db_session):
    ...
    item_data = ItemCreate(
        item_name="Test Product",
        item_unit="کلو",  # Using Urdu "کلو" instead of "KG"
        unit_price=100,
        stock_quantity=50
    )
    item = await create_items(db_session, item_data, user)
...
# ============================================
# FIXED TEST 1: CREATE UDHAR SUCCESS
# ============================================
@pytest.mark.asyncio
async def test_create_udhar_success(db_session):
    """Test successful udhar creation"""
...
# ============================================
# FIXED TEST 2: CREATE UDHAR NEW CUSTOMER
# ============================================
@pytest.mark.asyncio
async def test_create_udhar_new_customer(db_session):
    """Test creating udhar with new customer (auto-create)"""
    ...
    # Verify customer was created - using correct import
...
# ============================================
# FIXED TEST 3: DIFFERENT UNIT CONVERSION (Using correct Urdu units)
# ============================================
@pytest.mark.asyncio
async def test_create_udhar_different_unit_conversion(db_session):
    """Test creating udhar with different unit (e.g., grams instead of KG)"""
    ...
    # Use Urdu unit "گرام" (gram) which is compatible with "کلو" (KG)
    result = await create_udhar(
        db=db_session,
        customer_name="Test Customer",
        item_name="Test Product",
    ...
    assert result is not None
    # 500 grams = 0.5 KG, so total should be 50
    assert result["total_amount"] == 50  # 0.5 * 100
...
async def test_create_udhar_low_stock_alert(db_session):
    ...
    with patch('myapp.crud.udhaar_item.send_email') as mock_send_email:
        ...
        assert result is not None
        # Verify that add_task was called (for email)
        assert background_tasks.add_task.called


# ============================================
# FIXED TEST 5: UPDATE UDHAR ITEM SUCCESS
# ============================================
@pytest.mark.asyncio
async def test_update_udharitem_success(db_session):
    """Test successful udhar item update"""
...
# ============================================
# FIXED TEST 8: CREATE UDHAR EXACT STOCK LIMIT
# ============================================
@pytest.mark.asyncio
async def test_create_udhar_exact_stock_limit(db_session):
    """Test creating udhar with exact available stock"""
...
# ============================================
# ADDITIONAL TEST: UNIT CONVERSION WITH FRACTIONAL PREFIXES
# ============================================
@pytest.mark.asyncio
...
# ============================================
# ADDITIONAL TEST: CREATE UDHAR WITH DOZEN UNITS
# ============================================
@pytest.mark.asyncio
...
async def test_update_udharitem_change_item(db_session):
    ...
    # After creation: item1 stock = 50 - 5 = 45
    # item2 stock = 30
    ...
    # CORRECTED: Original item: 5 returned, so stock should be 50 (original) NOT 55
    # Because we already reduced to 45, then we return 5 = 50
...
async def test_delete_udharitem_success(db_session):
    ...
    # CORRECTED: Stock should be restored to original (50)
    updated_item = await read_item(db_session, item.item_id, user)
    assert updated_item.stock_quantity == original_stock  # 50
...
async def test_update_udharitem_same_quantity(db_session):
    ...
    # CORRECTED: Stock should remain 45 (not change)
    updated_item = await read_item(db_session, item.item_id, user)
    assert updated_item.stock_quantity == 45  # Still 45
