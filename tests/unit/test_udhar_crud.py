...
#     # May return None if marked as paid and no unpaid exists


# # ============================================
# # DELETE UDHAR BY ID TESTS
# # ============================================
# @pytest.mark.asyncio
# async def test_delete_udhar_by_id_success(db_session):
#     """Test successful udhar deletion"""
#     user, item = await create_test_user_and_item(db_session)
#     background_tasks = BackgroundTasks()
...
#     # Get the udhar
#     udhar = await get_udhar_by_customer(db_session, "Delete Customer", user)
...
#     # Verify deleted
#     udhars = await list_udhars(db_session, user)
#     assert len([u for u in udhars if u.customer_name == "Delete Customer"]) == 0
...
#     from myapp.crud.udhar_item import list_udharitems, delete_udharitem
#     items = await list_udharitems(db_session, user)
#     for item in items:
#         if item["customer_name"] == "Delete Item Customer":
#             await delete_udharitem(db_session, item["udharitem_id"], user)
#             break
...
#     assert udhar_after is None  # No items left
...
#     """Test concurrent direct additions to same customer"""
#     import asyncio
    
#     user, _ = await create_test_user_and_item(db_session)
    
#     async def add_amount(amount):
#         return await update_direct_addition(db_session, "Concurrent Customer", amount, user)
    
#     # Add amounts concurrently
#     amounts = [100, 200, 300]
#     results = await asyncio.gather(*[add_amount(a) for a in amounts])
    
#     assert len(results) == 3
#     # Final total should be 600
